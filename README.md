...
| **Public by design** | ✅ Landing page + sign-in/sign-up, `/api/health` (deep probe), and `/api/cron/health` (bearer `CRON_SECRET`, alerts via `ALERT_WEBHOOK_URL`). Uptime monitoring runs from GitHub Actions (`.github/workflows/uptime.yml`) — a scheduled probe of `https://gym-project-azure.vercel.app/api/health?deep=true` every 5 minutes that alerts on failure (replaces the removed Vercel cron, which Hobby allows only once/day). |
